...
color_angle_loss = tf.reduce_mean(
    angle(If[:, :, :, 0], vi_3[:, :, :, 0]) + angle(If[:, :, :, 1], vi_3[:, :, :, 1]) + angle(If[:, :, :, 2],
                                                                                              vi_3[:, :, :, 2]))
...
for epoch in range(start_epoch, epoch):
    for batch_id in range(start_step, numBatch):  # 总共的图片数目除以一个批数据10，所得的批数
        ...
        _, loss, loss_gradient, loss_exposure, loss_contrast, loss_color_mutual, loss_color_angle, loss_l1 = sess.run(
            [train_op, train_loss, gradient_loss, exposure_loss, contrast_loss, color_mutual_loss, color_angle_loss,
             l1_loss],
